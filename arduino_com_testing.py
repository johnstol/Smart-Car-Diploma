...
def avoid_fun():
	global sonar_serns,ser
	safe_to_avoid=70
	safe_to_stop=60
	back_obstacle_dist=30
	mf_threshold=8
	duration=0
	debug=True
	...
	while(sonar_serns[0]<safe_to_avoid and sonar_serns[1]<safe_to_avoid):
		...
		time.sleep(0.2)
		if (sonar_serns[2]<back_obstacle_dist or sonar_serns[3]<back_obstacle_dist):
			if(debug):
				print "Stop"
			break

	if  ((sonar_serns[0]<safe_to_avoid or sonar_serns[1]<safe_to_avoid) and (sonar_serns[2]<back_obstacle_dist or sonar_serns[3]<back_obstacle_dist)):
		if (debug):
	...
	#avoid, You can do it	
		...
		if (right_dist < left_dist):
			...
			start=time.time()
			while(duration<mf_threshold):
				#get distance
				sec_obst=sonar_serns[4]
				#check distance
				if(sec_obst<safe_to_stop):
					print'sonar:',sec_obst
					break
				time.sleep(0.2)	
			...
			#debug
			if (debug):
				if(duration>=mf_threshold):
					print('Duration:',duration)
				else:
					print('Sonar saw:',sec_obst)
